/*
api

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the PickCourseErrorMessageOrType type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &PickCourseErrorMessageOrType{}

// PickCourseErrorMessageOrType From T, pick a set of properties whose keys are in the union K
type PickCourseErrorMessageOrType struct {
	Message PickSchoolErrorMessageOrTypeMessage `json:"message"`
	Type    CourseErrorType                     `json:"type"`
}

type _PickCourseErrorMessageOrType PickCourseErrorMessageOrType

// NewPickCourseErrorMessageOrType instantiates a new PickCourseErrorMessageOrType object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewPickCourseErrorMessageOrType(message PickSchoolErrorMessageOrTypeMessage, type_ CourseErrorType) *PickCourseErrorMessageOrType {
	this := PickCourseErrorMessageOrType{}
	this.Message = message
	this.Type = type_
	return &this
}

// NewPickCourseErrorMessageOrTypeWithDefaults instantiates a new PickCourseErrorMessageOrType object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewPickCourseErrorMessageOrTypeWithDefaults() *PickCourseErrorMessageOrType {
	this := PickCourseErrorMessageOrType{}
	return &this
}

// GetMessage returns the Message field value
func (o *PickCourseErrorMessageOrType) GetMessage() PickSchoolErrorMessageOrTypeMessage {
	if o == nil {
		var ret PickSchoolErrorMessageOrTypeMessage
		return ret
	}

	return o.Message
}

// GetMessageOk returns a tuple with the Message field value
// and a boolean to check if the value has been set.
func (o *PickCourseErrorMessageOrType) GetMessageOk() (*PickSchoolErrorMessageOrTypeMessage, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Message, true
}

// SetMessage sets field value
func (o *PickCourseErrorMessageOrType) SetMessage(v PickSchoolErrorMessageOrTypeMessage) {
	o.Message = v
}

// GetType returns the Type field value
func (o *PickCourseErrorMessageOrType) GetType() CourseErrorType {
	if o == nil {
		var ret CourseErrorType
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *PickCourseErrorMessageOrType) GetTypeOk() (*CourseErrorType, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *PickCourseErrorMessageOrType) SetType(v CourseErrorType) {
	o.Type = v
}

func (o PickCourseErrorMessageOrType) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o PickCourseErrorMessageOrType) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["message"] = o.Message
	toSerialize["type"] = o.Type
	return toSerialize, nil
}

func (o *PickCourseErrorMessageOrType) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"message",
		"type",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varPickCourseErrorMessageOrType := _PickCourseErrorMessageOrType{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varPickCourseErrorMessageOrType)

	if err != nil {
		return err
	}

	*o = PickCourseErrorMessageOrType(varPickCourseErrorMessageOrType)

	return err
}

type NullablePickCourseErrorMessageOrType struct {
	value *PickCourseErrorMessageOrType
	isSet bool
}

func (v NullablePickCourseErrorMessageOrType) Get() *PickCourseErrorMessageOrType {
	return v.value
}

func (v *NullablePickCourseErrorMessageOrType) Set(val *PickCourseErrorMessageOrType) {
	v.value = val
	v.isSet = true
}

func (v NullablePickCourseErrorMessageOrType) IsSet() bool {
	return v.isSet
}

func (v *NullablePickCourseErrorMessageOrType) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullablePickCourseErrorMessageOrType(val *PickCourseErrorMessageOrType) *NullablePickCourseErrorMessageOrType {
	return &NullablePickCourseErrorMessageOrType{value: val, isSet: true}
}

func (v NullablePickCourseErrorMessageOrType) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullablePickCourseErrorMessageOrType) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
